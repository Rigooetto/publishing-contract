...
def import_coverage():
    ...
    with eng.connect() as c:
        ...
        # Sales type breakdown: revenue per (reporting_month, sales_type)
        # If two sales_types each show ~50% of total it means the CSV has duplicate coverage
        sales_type_rows = c.execute(_t("""
            SELECT reporting_month, sales_type, COUNT(*) AS row_count, SUM(total_net_revenue) AS rev
            FROM streaming_royalty
            GROUP BY reporting_month, sales_type
            ORDER BY reporting_month DESC, rev DESC
        """)).fetchall()
        sales_type_breakdown = {}
        for r in sales_type_rows:
            rm = r[0]
            if rm not in sales_type_breakdown:
                sales_type_breakdown[rm] = []
            sales_type_breakdown[rm].append({"sales_type": r[1], "rows": r[2], "rev": float(r[3])})
    ...
    return render_template_string(_COVERAGE_HTML,
        ...
        sales_type_breakdown=sales_type_breakdown,
        _sidebar_html=_sb())
...
@bp.route("/streaming-royalties/ald-inflation-check")
def ald_inflation_check():
    """Diagnostic: find ALL artists where ALD revenue > royalty_summary revenue (inflation scan)."""
    if auth_required():
        return redirect(url_for("publishing.login"))
    if role_required(_ADMIN_ONLY):
        flash("Access restricted.", "error")
    ...
    with eng.connect() as c:
        # Per-artist: compare ALD stored revenue vs the RS row it was built from.
        # For a clean artist, ald.net_revenue == rs.net_revenue (1.00×).
        # If artist_name_csv had the artist twice → ald.net_revenue == 2 × rs.net_revenue.
        inflated = c.execute(_t("""
            SELECT
                ald.artist_name,
                COUNT(DISTINCT ald.isrc)                                     AS isrc_count,
                SUM(ald.net_revenue)                                         AS ald_total,
                SUM(rs.net_revenue)                                          AS rs_total,
                ROUND(SUM(ald.net_revenue)::numeric /
                      NULLIF(SUM(rs.net_revenue)::numeric, 0), 3)            AS ratio
            FROM artist_label_detail ald
            JOIN royalty_summary rs
              ON rs.isrc             = ald.isrc
             AND rs.reporting_month  = ald.reporting_month
             AND rs.platform         = ald.platform
             AND rs.country          = ald.country
            GROUP BY ald.artist_name
            HAVING SUM(ald.net_revenue) > SUM(rs.net_revenue) * 1.05
            ORDER BY ratio DESC
            LIMIT 500
        """)).fetchall()

        # Total artist count in ALD for context
        total_artists_row = c.execute(_t(
            "SELECT COUNT(DISTINCT artist_name) FROM artist_label_detail"
        )).fetchone()
        total_artists = int(total_artists_row[0]) if total_artists_row else 0

    return render_template_string(_ALD_INFLATION_HTML,
        inflated=inflated,
...
<div style="padding:14px 18px;border-bottom:1px solid var(--b2);font-weight:600;font-size:14px">
  Revenue by Sales Type per Month
  <span style="font-size:12px;font-weight:400;color:var(--t3);margin-left:8px">If two sales types each show ~half the total, the CSV may contain duplicate coverage under different type names.</span>
</div>
<div class="tbl-wrap"><table class="tbl">
<thead><tr><th>Reporting Month</th><th>Sales Type</th><th class="num">Rows</th><th class="num">Revenue</th><th class="num">% of Month</th></tr></thead>
<tbody>
{% for rm, entries in sales_type_breakdown.items()|sort(reverse=true) %}
{% set month_total = entries|sum(attribute='rev') %}
{% for e in entries %}
<tr>
  <td>{{ rm.strftime('%B %Y') if rm else '—' }}</td>
  <td><strong>{{ e.sales_type or '—' }}</strong></td>
  <td class="num">{{ "{:,}".format(e.rows) }}</td>
  <td class="num">${{ "{:,.2f}".format(e.rev) }}</td>
  <td class="num">{{ "%.1f"|format(e.rev / month_total * 100 if month_total else 0) }}%</td>
</tr>
{% endfor %}
<tr style="background:var(--b1);font-weight:600">
  <td colspan="3" style="padding-left:24px;color:var(--t3);font-size:12px">{{ rm.strftime('%B %Y') if rm else '' }} total</td>
  <td class="num">${{ "{:,.2f}".format(month_total) }}</td>
  <td class="num">100%</td>
</tr>
{% endfor %}
</tbody></table></div>
...
<div class="ph-actions">
  <a href="/streaming-royalties/ald-inflation-check" class="btn btn-sec" style="margin-right:6px">Inflation Check (All Artists)</a>
  <a href="/streaming-royalties/imports" class="btn btn-sec">&#8592; Imports</a>
</div>
</div>
