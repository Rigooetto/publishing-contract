...
# ═══════════════════════════════════════════════════════════════
#  SHARED SIDEBAR JS  (no Jinja, no backticks, safe to embed)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
#  SHARED SIDEBAR HTML  — call sb_html(active) to get the block
# ═══════════════════════════════════════════════════════════════

def sb_html(active=""):
    links = [
        ("works_list",   "🎵", "Works"),
        ("formulario",   "✏️", "New Work"),
    ...
    res = (
        ...
        '<span class="sb-name">LabelMind</span>'
        '<span class="sb-toggle" id="sbToggle" onclick="toggleSidebar(event)" title="Toggle">&#9664;</span>'
        '</a>'
    ...
    for endpoint, icon, label in links:
        ...
        on_class = ' class="on"' if active == endpoint else ""
        res += '<a href="' + href + '"' + on_class + '><span class="ni">' + icon + '</span><span class="nl">' + label + "</span></a>"
    res += (
        "</nav>"
...
def topbar_html(active_pill=""):
    works_on = ' class="pill on"' if active_pill == "works" else ' class="pill"'
    sess_on = ' class="pill on"' if active_pill == "sessions" else ' class="pill"'
    return (
        '<header class="topbar">'
        '<div class="tb-search">🔍 Search works, sessions, writers\u2026<span class="tb-kbd">\u2318K</span></div>'
        '<div class="tb-right">'
        '<div class="pill-group">'
        '<a href="{{ url_for(\'works_list\') }}"' + works_on + ">Works</a>"
        '<a href="{{ url_for(\'batches_list\') }}"' + sess_on + ">Sessions</a>"
        "</div>"
        "{% if team_auth_enabled and session.get('logged_in') %}"
        '<a href="{{ url_for(\'logout\') }}" class="tb-ibtn" title="Log out">\U0001f6aa</a>'
        "{% endif %}"
...
LOGIN_HTML = (
    ...
    "<title>Login \u2014 LabelMind</title>"
    + _STYLE +
    "</head><body>"
    "<div class='login-wrap'><div class='login-card'>"
    "<div class='login-logo'><div class='login-logo-ico'>\U0001f3b5</div><span class='login-logo-name'>LabelMind</span></div>"
    ...
    "<input class='inp' type='password' name='password' required autocomplete='current-password' placeholder='\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022'></div>"
...
FORM_HTML = (
    "<!DOCTYPE html><html lang='en'><head>"
    "<meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
    "<title>New Work \u2014 LabelMind</title>"
    + _STYLE +
    "</head><body>"
    ...
    "<div class='flash-list'>{% for m in messages %}<div class='flash-item'>\u26a0\ufe0f {{ m }}</div>{% endfor %}</div>"
    "{% endif %}{% endwith %}"
    "<div class='ph'><div class='ph-left'><div class='ph-icon'>\u270f\ufe0f</div>"
    "<div><div class='ph-title'>New Work</div>"
    "<div class='ph-sub'>Create a work, add writers, and save into a session for contract generation.</div>"
    "</div></div></div>"
    "<div class='split-banner'>"
    "<div class='split-info'>"
    "<div class='split-lr'><span class='split-lbl'>Split Total</span>"
    "<span class='split-pct'><span id='splitTotal'>0.00</span>% Complete</span></div>"
    ...
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f4cb</div><span class='card-title'>Work Information</span></div>"
    "<div class='card-body'>"
    "<div class='g g3' style='margin-bottom:12px'>"
    "<div class='field'><label class='label'>Add to Existing Session</label>"
    "<div class='inp-wrap'><span class='inp-ico'>\U0001f4e6</span>"
    ...
    "</option>{% endfor %}"
    "</select></div></div>"
    "<div class='field'><label class='label'>Session Name</label>"
    "<div class='inp-wrap'><span class='inp-ico'>\U0001f3aa</span>"
    "<select class='inp' name='camp_id'>"
    "<option value=''>— Select existing session name</option>"
    "{% for camp in camps %}<option value='{{ camp.id }}'>{{ camp.name }}</option>{% endfor %}"
    "</select></div></div>"
    "<div class='field'><label class='label'>Or Create New Session Name</label>"
    "<input class='inp' name='new_camp_name' placeholder='New Session Name'></div>"
    "</div>"
    "<div class='g g2'>"
    "<div class='field'><label class='label'>Work Title</label>"
    "<div class='inp-wrap'><span class='inp-ico'>\U0001f3b5</span>"
    "<input class='inp' name='work_title' required placeholder='e.g. La Serenata'></div></div>"
    "<div class='field'><label class='label'>Contract Date</label>"
    "<div class='inp-wrap'><span class='inp-ico'>\U0001f4c5</span>"
    "<input class='inp' name='contract_date' type='date' required></div></div>"
    "</div></div></div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f465</div>"
    "<span class='card-title'>Writers</span>"
    "<div class='card-actions'><button type='button' class='btn btn-sec btn-sm' onclick='addWriter()'>+ Add Writer</button></div>"
    "</div>"
    "<div class='card-body' id='writerRows' style='padding-bottom:6px'></div></div>"
    ...
    "if(!fn&&!mn&&!ln){card.querySelector('.wc-dn').textContent='Writer '+(i+1);}"
    "});}"
    ...
    "var r=sel.closest('.wc');var p=proMap[sel.value];if(!p)return;"
    ...
    "return '<div class=\"ac-item\" data-w=\\''+JSON.stringify(w).replace(/\\\\/g,'\\\\\\\\').replace(/'/g,\"&#39;\")+'\\'>'+"
...
DUPLICATE_WARNING_HTML = (
    "<!DOCTYPE html><html lang='en'><head>"
    "<meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
    "<title>Possible Duplicate \u2014 LabelMind</title>"
    + _STYLE +
    "</head><body>"
    "<div class='app' id='mainApp'>"
    + sb_html("formulario") +
    ...
    "<div class='page'>"
    "<div class='ph'><div class='ph-left'><div class='ph-icon'>\u26a0\ufe0f</div>"
    "<div><div class='ph-title'>Possible Duplicate Found</div>"
    "<div class='ph-sub'>Existing works match this title and writer set.</div></div></div></div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f50d</div><span class='card-title'>Matching Works</span></div>"
    "<div class='card-body'>"
    "<table class='tbl' style='margin-bottom:18px'>"
    "<thead><tr><th>Title</th><th>Session</th><th>Created</th></tr></thead>"
    "<tbody>"
    "{% for item in duplicates %}"
    "<tr><td style='font-weight:600'>{{ item.title }}</td>"
    "<td>{{ item.camp_name or '\u2014' }}</td>"
    "<td style='color:var(--t2)'>{{ item.created_at }}</td></tr>"
    "{% endfor %}"
    "</tbody></table>"
    "<form method='post'>"
...
WORKS_LIST_HTML = (
    ...
    "<div class='page'>"
    "<div class='ph'>"
    "<div class='ph-left'><div class='ph-icon'>\U0001f3b5</div>"
    "<div><div class='ph-title'>Works</div><div class='ph-sub'>All registered musical works</div></div></div>"
    "<div class='ph-actions'><a href=\"{{ url_for('formulario') }}\" class='btn btn-primary'>+ New Work</a></div>"
    "</div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f50d</div><span class='card-title'>Search</span></div>"
    "<div class='card-body'><form method='get' style='display:flex;gap:8px'>"
    ...
    "</form></div></div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f4cb</div><span class='card-title'>All Works</span></div>"
    "<div class='tbl-wrap'><table class='tbl' style='min-width:960px'>"
    "<thead><tr>"
    "<th>Work Title</th><th>Session</th><th>Contract Date</th><th>Writers</th>"
    "<th>Contract \U0001f4dd</th><th>Signed PDF \U0001f4d1</th><th>DS Status</th><th>Signed</th>"
    "<th>Created</th><th></th>"
    "</tr></thead>"
    "<tbody>"
    "{% for work in works %}"
    ...
    "<a href='{{ first_doc.drive_web_view_link }}' target='_blank' class='file-link' title='{{ first_doc.file_name }}'>\U0001f4dd {{ first_doc.file_name | truncate(26,true,'\u2026') }}</a>"
    "{% elif docs|length > 1 %}"
    "<a href=\"{{ url_for('work_detail', work_id=work.id) }}\" class='btn btn-cyan btn-xs'>\U0001f4dd {{ docs|length }} docs</a>"
    "{% else %}\u2014{% endif %}"
    "</td>"
    "<td>"
    "{% set signed_docs = [] %}"
    ...
    "<a href='{{ signed_docs[0].signed_pdf_drive_web_view_link }}' target='_blank' class='file-link'>\U0001f4d1 Signed PDF</a>"
    "{% elif signed_docs|length > 1 %}"
    "<a href=\"{{ url_for('work_detail', work_id=work.id) }}\" class='btn btn-success btn-xs'>\U0001f4d1 {{ signed_docs|length }}</a>"
    "{% else %}\u2014{% endif %}"
    "</td>"
    "<td>"
...
BATCHES_LIST_HTML = (
    "<!DOCTYPE html><html lang='en'><head>"
    "<meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
    "<title>Sessions \u2014 LabelMind</title>"
    + _STYLE +
    "</head><body>"
    "<div class='app' id='mainApp'>"
    + sb_html("batches_list") +
    "<main class='main'>"
    + topbar_html("sessions") +
    "<div class='page'>"
    "<div class='ph'>"
    "<div class='ph-left'><div class='ph-icon'>\U0001f4e6</div>"
    "<div><div class='ph-title'>Sessions</div><div class='ph-sub'>Groups of works ready for contract generation</div></div></div>"
    "<div class='ph-actions'><a href=\"{{ url_for('formulario') }}\" class='btn btn-primary'>+ New Work</a></div>"
    "</div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f4e6</div><span class='card-title'>All Sessions</span></div>"
...
BATCH_DETAIL_HTML = (
    "<!DOCTYPE html><html lang='en'><head>"
    "<meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
    "<title>Session {{ batch.id }} \u2014 LabelMind</title>"
    + _STYLE +
    "</head><body>"
    "<div class='app' id='mainApp'>"
    + sb_html("batches_list") +
    ...
    "<div class='ph'>"
    "<div class='ph-left'><div class='ph-icon'>\U0001f4e6</div>"
    "<div><div class='ph-title'>Session #{{ batch.id }}</div>"
    ...
    "</button></form>"
    "</div></div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\u2139\ufe0f</div><span class='card-title'>Session Info</span></div>"
    ...
    "</div></div></div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f3b5</div><span class='card-title'>Works in Session</span></div>"
    "<div class='tbl-wrap'><table class='tbl'>"
    ...
    "</tbody></table></div></div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f465</div><span class='card-title'>Writer Summary</span></div>"
    "<div class='tbl-wrap'><table class='tbl'>"
    ...
    "</tbody></table></div></div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f4c4</div><span class='card-title'>Generated Documents</span></div>"
    "<div class='tbl-wrap'><table class='tbl' style='min-width:860px'>"
    "<thead><tr>"
    "<th>Writer</th><th>Type</th><th>File (click to open)</th><th>Generated</th>"
    ...
    "<a href='{{ doc.drive_web_view_link }}' target='_blank' class='file-link' title='{{ doc.file_name }}'>\U0001f4dd {{ doc.file_name | truncate(30,true,'\u2026') }}</a>"
    "{% else %}<span class='file-link-plain'>{{ doc.file_name }}</span>{% endif %}"
    "</td>"
    ...
    "<span class='spin ds-spin'></span></button></form>"
    "</td>"
    ...
    "</td>"
    "<td>"
    "{% if doc.signed_pdf_drive_web_view_link %}<a href='{{ doc.signed_pdf_drive_web_view_link }}' target='_blank' class='file-link'>\U0001f4d1 Signed</a>"
    "{% elif doc.signed_web_view_link %}<a href='{{ doc.signed_web_view_link }}' target='_blank' class='file-link'>\U0001f4d1 Signed</a>"
    "{% else %}\u2014{% endif %}"
    "</td>"
    ...
    "return '<a href=\"'+doc.drive_web_view_link+'\" target=\"_blank\" class=\"file-link\" title=\"'+name+'\">\U0001f4dd '+esc(short)+'</a>';}"
    ...
    "var signedCell='\u2014';"
    "if(doc.signed_pdf_drive_web_view_link)signedCell='<a href=\"'+doc.signed_pdf_drive_web_view_link+'\" target=\"_blank\" class=\"file-link\">\U0001f4d1 Signed</a>';"
    "else if(doc.signed_web_view_link)signedCell='<a href=\"'+doc.signed_web_view_link+'\" target=\"_blank\" class=\"file-link\">\U0001f4d1 Signed</a>';"
    ...
    "var btn=document.getElementById('genBtn');if(!btn)return;"
    "btn.disabled=false;"
    ...
    "e.preventDefault();"
    "var btn=f.querySelector('.ds-btn'),spin=f.querySelector('.ds-spin'),lbl=f.querySelector('.ds-lbl');"
    "if(btn)btn.disabled=true;if(spin)spin.classList.add('on');if(lbl)lbl.textContent='Sending\u2026';"
...
WORK_DETAIL_HTML = (
    "<!DOCTYPE html><html lang='en'><head>"
    "<meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
    "<title>{{ work.title }} \u2014 LabelMind</title>"
    + _STYLE +
    "</head><body>"
    "<div class='app' id='mainApp'>"
    + sb_html("works_list") +
    ...
    "<div class='ph'>"
    "<div class='ph-left'><div class='ph-icon'>\U0001f3b5</div>"
    "<div><div class='ph-title'>{{ work.title }}</div>"
    ...
    "</div></div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f4cb</div><span class='card-title'>Work Info</span></div>"
    "<div class='card-body'><div class='info-grid'>"
    ...
    "</div></div></div>"
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f465</div><span class='card-title'>Writers &amp; Splits</span></div>"
    "<div class='tbl-wrap'><table class='tbl'>"
    ...
    "<div class='card'><div class='card-hd'><div class='card-ico'>\U0001f4c4</div><span class='card-title'>Generated Documents</span></div>"
    "<div class='tbl-wrap'><table class='tbl' style='min-width:860px'>"
    "<thead><tr><th>Writer</th><th>Type</th><th>File (click to open)</th><th>Generated At</th><th>DocuSign</th><th>DS Status</th><th>Certificate</th><th>Signed PDF</th><th>Status</th></tr></thead>"
    "<tbody>"
    "{% for doc in documents %}"
    "<tr data-doc-id='{{ doc.id }}'>"
    "<td style='font-weight:600;white-space:nowrap'>{{ doc.writer_name_snapshot }}</td>"
...
def get_batch_writer_summary(batch_id):
    work_writers = (
        WorkWriter.query.join(Work).filter(Work.batch_id == batch_id).all()
    )
    grouped = {}
...
def docusign_webhook():
    ...
    try:
        ...
        app.logger.warning("WEBHOOK ENVELOPE ID: %s", envelope_id)
        app.logger.warning("WEBHOOK STATUS: %s", raw_status)
...
def send_document_docusign(document_id):
    ...
    try:
        ...
        app.logger.warning("DOCUSIGN SEND START document_id=%s", document_id)
        app.logger.warning("DOCUSIGN SEND writer_email=%s", getattr(writer, "email", None))
        app.logger.warning("DOCUSIGN SEND drive_file_id=%s", getattr(document, "drive_file_id", None))
...
if __name__ == "__main__":
    app.run(debug=True, host="0.0.0.0", port=int(os.getenv("PORT", "5052")))GOOGLE_SERVICE_ACCOUNT_JSON = os.getenv("GOOGLE_SERVICE_ACCOUNT_JSON", "")
